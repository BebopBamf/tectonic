...
#define PERROR(result)                                                         \
        if ((result) == -1) {                                                  \
                perror("texpresso_fork_with_channel failure (" __FILE__        \
                       ":" SSTR(__LINE__) ")");                                \
                abort();                                                       \
        }
...
#define PASSERT(command, ...)                                                  \
        if (!(command)) {                                                      \
                fprintf(stderr,                                                \
                        "texpresso_fork_with_channel failure (" #command       \
                        ") " __VA_ARGS__);                                     \
                abort();                                                       \
        }

static void send_child_fd(int chan_fd, int32_t pid, uint32_t time,
                          int child_fd) {
        ssize_t sent;
...
int texpresso_fork_with_channel(int fd, uint32_t time) {
        ...
        if (child == 0) {
                ...
                do {
                        ...
                        // Ignore any flush message, the buffers have been
                        // flushed anyway before starting the fork.
                } while (recvd == 4 && answer[0] == 'F' && answer[1] == 'L' &&
                         answer[2] == 'S' && answer[3] == 'H');

                PASSERT(recvd == 4 && answer[0] == 'D' && answer[1] == 'O' &&
                            answer[2] == 'N' && answer[3] == 'E',
                        "recvd: %d, answer: %C%C%C%C", recvd, answer[0],
