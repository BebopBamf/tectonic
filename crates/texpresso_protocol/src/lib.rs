...
extern "C" {
    fn texpresso_fork_with_channel(fd: libc::c_int, time: u32) -> libc::c_int;
}
...
impl Client {
    ...
    fn flush_pending(&mut self) {
        ...
        if self.write_len != 0 {
            ...
            self.io
                .write(self.write, self.write_pos, &self.write_buf[0..len], &[]);
            self.write_len = 0;
            self.write = -1;
        }
    }
    ...
    pub fn write(&mut self, file: FileId, pos: u32, buf: &[u8]) {
        ...
        if (self.write_len as usize + buf.len()) <= BUF_SIZE {
            ...
            self.io.write(
                file,
                self.write_pos,
                &self.write_buf[0..self.write_len as usize],
                buf,
            );
            self.write_len = 0;
        }
