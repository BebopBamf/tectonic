...
void *xmalloc(size_t size) {
        ...
        if (new_mem == NULL)
                _tt_abort("xmalloc request for %lu bytes failed",
                          (unsigned long)size);
...
void *xrealloc(void *old_ptr, size_t size) {
        ...
        if (old_ptr == NULL) {
                ...
                if (new_mem == NULL)
                        _tt_abort("xrealloc() to %lu bytes failed",
                                  (unsigned long)size);
        }
...
PRINTF_FUNC(2, 0)
void ttstub_diag_vprintf(ttbc_diagnostic_t *diag, const char *format,
                         va_list ap) {
...
const char *_ttbc_get_error_message(void) { return format_buf; }

ttbc_state_t *_ttbc_get_core_state(void) { return tectonic_global_bridge_core; }
...
void ttbc_global_engine_exit(void) { tectonic_global_bridge_core = NULL; }
...
PRINTF_FUNC(1, 2) void ttstub_issue_error(const char *format, ...) {
        ...
        va_start(ap, format);
        vsnprintf(format_buf, BUF_SIZE, format,
                  ap); /* Not ideal to (ab)use format_buf here */
        va_end(ap);
...
size_t ttstub_output_write(rust_output_handle_t handle, const char *data,
                           size_t len) {
        return ttbc_output_write(tectonic_global_bridge_core, handle,
...
rust_input_handle_t ttstub_input_open(char const *path, ttbc_file_format format,
                                      int is_gz) {
        return ttbc_input_open(tectonic_global_bridge_core, path, format,
                               is_gz);
}
...
ssize_t ttstub_get_last_input_abspath(char *buffer, size_t len) {
        return ttbc_get_last_input_abspath(tectonic_global_bridge_core,
                                           (uint8_t *)buffer, len);
}
...
time_t ttstub_input_get_mtime(rust_input_handle_t handle) {
        /* Due to the Musl 1.2 "time64" transition, we can't safely bridge
         * time_t between Rust and C code. And formally, ISO C provides nearly
         * no guarantees about what the type time_t actually is. So let's just
...
size_t ttstub_input_seek(rust_input_handle_t handle, ssize_t offset,
        ...
        size_t rv = ttbc_input_seek(tectonic_global_bridge_core, handle, offset,
                                    whence, &internal_error);
...
ssize_t ttstub_input_read(rust_input_handle_t handle, char *data, size_t len) {
        return ttbc_input_read(tectonic_global_bridge_core, handle,
                               (uint8_t *)data, len);
}

ssize_t ttstub_input_read_partial(rust_input_handle_t handle, char *data,
                                  size_t len) {
        return ttbc_input_read_partial(tectonic_global_bridge_core, handle,
                                       (uint8_t *)data, len);
}
...
int ttstub_get_file_md5(char const *path, char *digest) {
        return ttbc_get_file_md5(tectonic_global_bridge_core, path,
                                 (uint8_t *)digest);
}
...
int ttstub_pic_get_cached_bounds(const char *name, int type, int page,
                                 float bounds[4]) {
        return ttbc_pic_get_cached_bounds(tectonic_global_bridge_core, name,
                                          type, page, bounds);
}

void ttstub_pic_set_cached_bounds(const char *name, int type, int page,
                                  float bounds[4]) {
        ttbc_pic_set_cached_bounds(tectonic_global_bridge_core, name, type,
                                   page, bounds);
}
